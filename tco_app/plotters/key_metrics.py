...
def create_key_metrics_chart(bev_results, diesel_results):
    ...
    if (
        "infrastructure_costs" in bev_results
        and "annual_kms" in bev_results
        and "truck_life_years" in bev_results
    ):
        if "npv_per_vehicle_with_incentives" in bev_results["infrastructure_costs"]:
            infra_npv = bev_results["infrastructure_costs"][
    ...
    metrics = {
        ...
        "Maintenance per km": [
            bev_results["annual_costs"]["annual_maintenance_cost"]
            / bev_results["annual_kms"],
            diesel_results["annual_costs"]["annual_maintenance_cost"]
            / diesel_results["annual_kms"],
        ],
        "CO₂ per km": [
    ...
    normalised = {
        c: [
            safe_division(v, max, context="v/max calculation")(
                bev_values[i], diesel_values[i]
            )
            for v in metrics[c]
        ]
