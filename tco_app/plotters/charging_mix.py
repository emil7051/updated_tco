...
def create_charging_mix_chart(bev_results):
	...
	charging_mix = {cid: bev_results['charging_mix'][cid] for cid in bev_results['charging_mix'] if bev_results['charging_mix'][cid] > 0}
	weighted_price = weighted_electricity_price(charging_mix, bev_results['charging_options']) if sum(values) else None
	subtitle = f'Weighted Average: ${weighted_price:.2f}/kWh' if weighted_price else 'No charging mix data'

	fig.update_layout(
		title={'text': f'Charging Mix Distribution<br><sup>{subtitle}</sup>', 'x': 0.5, 'y': 0.95, 'xanchor': 'center', 'yanchor': 'top'},
		height=400,
	)
	return fig 