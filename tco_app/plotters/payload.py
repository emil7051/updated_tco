...
def create_payload_comparison_chart(payload_metrics, bev_results, diesel_results):
	...
	fig = go.Figure()
	fig.add_trace(go.Bar(x=['Diesel'], y=[diesel_results['tco']['npv_total_cost']], name='Total TCO', marker_color='#ff7f0e'))
	fig.add_trace(go.Bar(
		x=['BEV (Standard)', 'BEV (Payload-Adjusted)'],
		y=[bev_results['tco']['npv_total_cost'], bev_results['tco']['npv_total_cost']],
		name='Base TCO',
		marker_color='#1f77b4',
	))
...
def create_payload_sensitivity_chart(bev_results, diesel_results, financial_params, distances):
	...
	for distance in distances:
		bev_temp, diesel_temp = copy.deepcopy(bev_results), copy.deepcopy(diesel_results)
		...
		results.append({
			...
			'adjusted_tco_ratio': (
				payload_metrics['bev_adjusted_lifetime_tco'] / diesel_temp['tco']['npv_total_cost']
				if payload_metrics['has_penalty'] else bev_temp['tco']['npv_total_cost'] / diesel_temp['tco']['npv_total_cost']
			),
		})

	results_df = pd.DataFrame(results)
	fig = go.Figure()
	fig.add_trace(go.Scatter(x=results_df['distance'], y=results_df['standard_tco_ratio'], mode='lines+markers', name='Standard TCO Ratio (BEV/Diesel)', line=dict(color='#1f77b4')))
	fig.add_trace(go.Scatter(x=results_df['distance'], y=results_df['adjusted_tco_ratio'], mode='lines+markers', name='Payload-Adjusted TCO Ratio', line=dict(color='#d62728')))

	fig.add_shape(type='line', x0=min(distances), y0=1, x1=max(distances), y1=1, line=dict(color='green', width=2, dash='dash'))
	fig.add_annotation(x=min(distances) + (max(distances) - min(distances)) * 0.05, y=1.02, text='Break-even point (BEV = Diesel)', showarrow=False, font=dict(color='green'))
