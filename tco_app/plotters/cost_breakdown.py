...
def create_cost_breakdown_chart(bev_results, diesel_results):
	...
	df = pd.DataFrame({
		...
		'Vehicle Type': [Drivetrain.BEV.value] * len(categories) + [Drivetrain.DIESEL.value] * len(categories),
	})
...
# Initial cumulative costs include acquisition (and infrastructure for BEV)
	...
	for year in range(1, truck_life_years):
		...
		if 'infrastructure_costs' in bev_results:
			infra_maintenance = bev_results['infrastructure_costs']['annual_maintenance'] / bev_results['infrastructure_costs'].get('fleet_size', 1)
	...
	df = pd.DataFrame({
		...
		'Vehicle Type': [Drivetrain.BEV.value] * len(years) + [Drivetrain.DIESEL.value] * len(years),
	})
