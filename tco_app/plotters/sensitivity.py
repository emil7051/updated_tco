...
def create_sensitivity_chart(bev_results, diesel_results, parameter, param_range, recalculated_tcos):
	...
	fig.add_trace(go.Scatter(
		x=param_range,
		y=[t['bev']['tco_lifetime'] - t['diesel']['tco_lifetime'] for t in recalculated_tcos],
		mode='lines+markers',
	...
	for i in range(len(param_range) - 1):
		bev1, diesel1 = recalculated_tcos[i]['bev']['tco_lifetime'], recalculated_tcos[i]['diesel']['tco_lifetime']
		bev2, diesel2 = recalculated_tcos[i + 1]['bev']['tco_lifetime'], recalculated_tcos[i + 1]['diesel']['tco_lifetime']
	...
	if break_even_value is not None:
		bev_at_be = next(
			(b['bev']['tco_lifetime'] for b in recalculated_tcos if b['bev']['tco_lifetime'] - b['diesel']['tco_lifetime'] == 0),
			None,
		)
		fig.add_trace(go.Scatter(
			x=[break_even_value],
