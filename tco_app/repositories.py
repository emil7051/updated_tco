...
from tco_app.src.exceptions import VehicleNotFoundError # Assuming this exception exists
...
class VehicleRepository:
    ...
    def get_vehicle_by_id(self, vehicle_id: str) -> pd.Series:
        ...
        if self.vehicle_models_df.empty or 'vehicle_id' not in self.vehicle_models_df.columns:
            raise VehicleNotFoundError(f"Vehicle models data is empty or 'vehicle_id' column is missing.")
        
        vehicle_data = self.vehicle_models_df[self.vehicle_models_df['vehicle_id'] == vehicle_id]
        if vehicle_data.empty:
            raise VehicleNotFoundError(f"Vehicle with ID '{vehicle_id}' not found in vehicle_models.")
    ...
    def get_fees_by_vehicle_id(self, vehicle_id: str) -> pd.Series:
        ...
        if self.vehicle_fees_df.empty or 'vehicle_id' not in self.vehicle_fees_df.columns:
            # Allow returning empty series if fees are optional or not always present for a vehicle
            return pd.Series(dtype=object) 
            
        fees_data = self.vehicle_fees_df[self.vehicle_fees_df['vehicle_id'] == vehicle_id]
        if fees_data.empty:
...
class ParametersRepository:
    ...
    def get_externalities_data(self) -> pd.DataFrame:
        return self.data_tables.get('externalities', pd.DataFrame()).copy() # Changed from 'externalities_data' to 'externalities' to match orchestrator
