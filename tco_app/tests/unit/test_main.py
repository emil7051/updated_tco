...
def test_main_import_triggers_render():
    ...
    with patch("tco_app.src.st.set_page_config"), \
         patch("tco_app.src.st.sidebar.title"), \
         patch("tco_app.src.st.sidebar.radio", return_value=page_key), \
         patch("importlib.import_module") as import_module_mock, \
         patch("tco_app.main.st.sidebar.radio", return_value=page_key):
