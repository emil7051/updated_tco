...
class TestValidateDataFrameColumns:
    ...
    def test_single_missing_column(self, sample_df):
        ...
        with pytest.raises(ValueError) as exc_info:
            validate_dataframe_columns(sample_df, ['col1', 'missing_col'], "test DataFrame")
        
        error_msg = str(exc_info.value)
        assert "test DataFrame is missing required columns: {'missing_col'}" in error_msg
