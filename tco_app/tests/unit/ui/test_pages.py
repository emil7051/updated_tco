...
class _StreamlitStub(ModuleType):
	def __getattr__(self, name):
		...
		elif name in {'metric', 'plotly_chart', 'markdown', 'subheader', 'info', 'warning', 'caption', 'title', 'header', 'text', 'download_button', 'dataframe', 'success', 'experimental_set_query_params'}:
...
def _smoke_render(module_path: str) -> None:
	...
	_ctx.get_context = lambda: {
		'bev_results': {
			...
			'infrastructure_costs': {'infrastructure_price': 0, 'annual_maintenance': 0, 'npv_per_vehicle': 0, 'service_life_years': 0, 'replacement_cycles': 0},
			'charging_requirements': {'daily_kwh_required': 0, 'charging_time_per_day': 0, 'charger_power': 0, 'max_vehicles_per_charger': 1},
			'tco': {},
			'annual_costs': {},
		},
	...
	_fin.calculate_payload_penalty_costs = lambda *a, **k: {'has_penalty': False, 'payload_difference': 0, 'payload_difference_percentage': 0}
