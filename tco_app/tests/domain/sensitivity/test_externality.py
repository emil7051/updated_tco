...
def mock_diesel_vehicle_data():
    return pd.Series({"vehicle_id": 2, "drivetrain": "Diesel", "vehicle_class": "Articulated"})
...
def test_perform_externality_sensitivity_default_range(
    ...
    with patch("tco_app.domain.sensitivity.externality.calculate_externalities") as mock_calc_ext, \
         patch("tco_app.domain.sensitivity.externality.calculate_social_tco") as mock_calc_social_tco:

        def side_effect_calc_ext(vehicle_data, modified_ext_df, an_kms, tr_life, disc_rate):
            ...
            if vehicle_data["drivetrain"] == "BEV":
                ...
                return {"externality_per_km": val, "total_externalities": total_ext * 0.5}
            else:  # Diesel
                # Diesel externalities are 100% of the modified base
                val = base_cost_per_km * 1.0
                return {"externality_per_km": val, "total_externalities": total_ext * 1.0}

        def side_effect_calc_social_tco(tco_results, ext_results):
            return {
                "social_tco_per_km": tco_results["tco_per_km"] + ext_results["externality_per_km"],
                "social_tco_lifetime": tco_results["tco_lifetime"] + ext_results["total_externalities"]
            }
        ...
        for i, res_item in enumerate(results):
            ...
            assert res_item["bev_externality_per_km"] == pytest.approx(expected_bev_ext_per_km)
            assert res_item["diesel_externality_per_km"] == pytest.approx(expected_diesel_ext_per_km)

            assert res_item["bev_tco_per_km"] == mock_bev_results["tco"]["tco_per_km"]
            assert res_item["diesel_tco_per_km"] == mock_diesel_results["tco"]["tco_per_km"]

            expected_bev_social_tco_per_km = mock_bev_results["tco"]["tco_per_km"] + expected_bev_ext_per_km
            expected_diesel_social_tco_per_km = mock_diesel_results["tco"]["tco_per_km"] + expected_diesel_ext_per_km
            assert res_item["bev_social_tco_per_km"] == pytest.approx(expected_bev_social_tco_per_km)
            assert res_item["diesel_social_tco_per_km"] == pytest.approx(expected_diesel_social_tco_per_km)

            # Simplified total externalities for abatement cost check
            bev_total_ext = (modified_input_cost * 0.5) * annual_kms * truck_life_years
            diesel_total_ext = (modified_input_cost * 1.0) * annual_kms * truck_life_years
            
            bev_social_lifetime = mock_bev_results["tco"]["tco_lifetime"] + bev_total_ext
            diesel_social_lifetime = mock_diesel_results["tco"]["tco_lifetime"] + diesel_total_ext

            emission_savings = mock_diesel_results["emissions"]["lifetime_emissions"] - mock_bev_results["emissions"]["lifetime_emissions"]
            if emission_savings > 0:
                expected_abatement_cost = (bev_social_lifetime - diesel_social_lifetime) / (emission_savings / 1000)
            else:
                expected_abatement_cost = float("inf")
            assert res_item["social_abatement_cost"] == pytest.approx(expected_abatement_cost)
...
def test_perform_externality_sensitivity_custom_range(
    ...
    with patch("tco_app.domain.sensitivity.externality.calculate_externalities", MagicMock(return_value={"externality_per_km": 0.05, "total_externalities": 10000})) as mock_calc_ext, \
         patch("tco_app.domain.sensitivity.externality.calculate_social_tco", MagicMock(return_value={"social_tco_per_km": 1.05, "social_tco_lifetime": 110000})) as mock_calc_social_tco:
...
def test_perform_externality_sensitivity_zero_emission_savings(
    ...
    with patch("tco_app.domain.sensitivity.externality.calculate_externalities", MagicMock(return_value={"externality_per_km": 0.05, "total_externalities": 10000})), \
         patch("tco_app.domain.sensitivity.externality.calculate_social_tco", MagicMock(return_value={"social_tco_per_km": 1.05, "social_tco_lifetime": 110000})):
        results = perform_externality_sensitivity(
            bev_results=mock_bev_results,
            diesel_results=mock_diesel_results_modified,
            externalities_data=mock_externalities_data,
            annual_kms=10000, truck_life_years=10, discount_rate=0.05,
...
def test_perform_externality_sensitivity_negative_emission_savings(
    ...
    mock_bev_results_modified["emissions"] = {"lifetime_emissions": mock_diesel_results["emissions"]["lifetime_emissions"] + 1000}
