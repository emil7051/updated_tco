...
def test_weighted_electricity_price_golden(mix, prices, expected):
	assert math.isclose(weighted_electricity_price(mix, prices), expected, rel_tol=1e-12)
...
def test_price_parity_year_golden(bev_curve: List[float], diesel_curve: List[float], expected: float):
	assert math.isclose(price_parity_year(bev_curve, diesel_curve), expected, rel_tol=1e-9)
...
def test_calculate_residual_value_basic():
	...
	res = calculate_residual_value(vehicle, years=5, initial_depreciation=0.1, annual_depreciation=0.05)
