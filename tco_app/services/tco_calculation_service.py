...
from tco_app.domain import energy, finance, externalities # battery is not directly used by model_runner top level
from tco_app.domain.sensitivity import metrics as sensitivity_metrics # Added import for calculate_comparative_metrics
...
from tco_app.src.utils.battery import calculate_battery_replacement # Used by model_runner
...
class TCOCalculationService:
    def __init__(self, 
                 ...
                 params_repo: ParametersRepository 
                ):
    ...
    def calculate_single_vehicle_tco(self, request: CalculationRequest) -> TCOResult:
        ...
        """
        logger.info(f"Starting TCO calculation for vehicle: {request.vehicle_data.get(DataColumns.VEHICLE_ID, 'N/A')}")
        ...
        try:
            ...
            return self._assemble_tco_result(
                request, energy_costs, annual_costs, acquisition_costs, 
                battery_costs, externalities, infrastructure_costs
            )

        except Exception as e:
            vehicle_id_for_error = request.vehicle_data.get(DataColumns.VEHICLE_ID, 'N/A') if request and hasattr(request, 'vehicle_data') else 'N/A'
            logger.error(
    ...
    def _calculate_energy_costs(self, request: CalculationRequest) -> Dict[str, Any]:
        ...
        energy_cost_per_km = energy.calculate_energy_costs(
            ...
            request.parameters.charging_mix if request.drivetrain == Drivetrain.BEV else None,
        )
    ...
    def _calculate_annual_costs(self, request: CalculationRequest, energy_costs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _calculate_acquisition_costs(self, request: CalculationRequest) -> Dict[str, Any]:
        ...
        initial_dep, annual_dep = get_residual_value_parameters(request.financial_params)
    ...
    def _calculate_battery_costs(self, request: CalculationRequest) -> Dict[str, Any]:
        ...
        return {
            'npv_battery_replacement_cost': npv_battery_replacement_cost
        }
    ...
    def _calculate_infrastructure_costs(self, request: CalculationRequest) -> Dict[str, Any]:
        ...
        if request.drivetrain == Drivetrain.BEV:
            ...
            selected_infra_df = request.infrastructure_options[
                request.infrastructure_options[DataColumns.INFRASTRUCTURE_ID] == request.parameters.selected_infrastructure_id
            ]
            if selected_infra_df.empty:
                raise CalculationError(f"Selected infrastructure ID {request.parameters.selected_infrastructure_id} not found.")
    ...
    def _assemble_tco_result(self, 
                           ...
                           infrastructure_costs: Dict[str, Any]) -> TCOResult:
        ...
        social_tco_total_lifetime = (
            tco_total_lifetime_final + 
            externalities.get('npv_externality', 0.0)
        )
        ...
        return TCOResult(
            vehicle_id=request.vehicle_data.get(DataColumns.VEHICLE_ID, 'unknown_vehicle'),
            ...
            annual_operating_cost=annual_costs['annual_costs_result']['annual_operating_cost'],
            energy_cost_per_km=energy_costs['energy_cost_per_km'],
            
            lifetime_emissions_co2e=energy_costs['emissions_result']['lifetime_emissions'],
            ...
            infrastructure_costs_breakdown=infrastructure_costs['infrastructure_costs_breakdown'],
            weighted_electricity_price=weighted_elec_price,

            scenario_meta={'name': request.parameters.scenario_name},
        )

    def _calculate_tco_metrics(self, tco_total_lifetime: float, request: CalculationRequest) -> tuple[float, float]:
        """Calculate per-km and per-tonne-km TCO metrics."""
        tco_per_km = 0.0
        tco_per_tonne_km = 0.0
        
        if request.parameters.annual_kms > 0 and request.parameters.truck_life_years > 0:
            total_lifetime_kms = request.parameters.annual_kms * request.parameters.truck_life_years
    ...
    def compare_vehicles(self, 
                         ...
                         comparison_vehicle_request: CalculationRequest) -> ComparisonResult:
        """
        ...
        """
        logger.info(f"Starting TCO comparison between {base_vehicle_request.vehicle_data.get(DataColumns.VEHICLE_ID, 'Base')} and {comparison_vehicle_request.vehicle_data.get(DataColumns.VEHICLE_ID, 'Comparison')}")

        base_tco_result = self.calculate_single_vehicle_tco(base_vehicle_request)
        comparison_tco_result = self.calculate_single_vehicle_tco(comparison_vehicle_request)
        ...
        base_tco_result_dict = convert_tco_result_to_model_runner_dict(base_tco_result, base_vehicle_request)
        comparison_tco_result_dict = convert_tco_result_to_model_runner_dict(comparison_tco_result, comparison_vehicle_request)
        ...
        truck_life_years_for_comparison = base_vehicle_request.parameters.truck_life_years
        ...
        tco_savings = comparison_tco_result.tco_total_lifetime - base_tco_result.tco_total_lifetime
        ...
        logger.info(f"TCO comparison completed using domain.sensitivity.metrics.calculate_comparative_metrics. Lifetime TCO Savings ({base_tco_result.vehicle_id} vs {comparison_tco_result.vehicle_id}): {tco_savings:.2f}")
