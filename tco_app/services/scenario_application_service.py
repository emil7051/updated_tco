...
class ScenarioApplicationService:
    ...
    def apply_scenario_by_id(
        ...
        selected_params = scenario_params[
    ...
    def _should_apply_modification(
        ...
        # Vehicle type check
        if (mod.vehicle_type != 'All' and 
            target_vehicle_type and 
            mod.vehicle_type != target_vehicle_type):
            return False
        
        # Drivetrain check
        if (mod.vehicle_drivetrain != 'All' and 
            target_drivetrain and 
            mod.vehicle_drivetrain != target_drivetrain):
            return False
    ...
    def _apply_single_modification(
        ...
        if mod.table_name not in tables:
            raise ScenarioError(
                f"Table '{mod.table_name}' not found in data tables"
            )
        ...
        if mod.table_name == 'financial_params':
            ...
            logger.warning(
                f"No handler for table '{mod.table_name}'"
            )
    ...
    def _apply_financial_param(
        ...
        if mask.any():
            ...
            if mod.parameter_name == 'diesel_default_price':
                ...
                logger.warning(
                    f"Financial parameter '{mod.parameter_name}' not found"
                )
    ...
    def _apply_battery_param(
        ...
        if mask.any():
            ...
            logger.warning(
                f"Battery parameter '{mod.parameter_name}' not found"
            )
    ...
    def _apply_vehicle_modifier(
        ...
        if mod.parameter_name == 'msrp_price_modifier':
            ...
            logger.warning(
                f"Unknown vehicle modifier: {mod.parameter_name}"
            )
            return
