...
def safe_iloc_zero(
    df: pd.DataFrame, condition: pd.Series, context: str = "data"
) -> pd.Series:
...
def safe_lookup_vehicle(vehicle_models: pd.DataFrame, vehicle_id: str) -> pd.Series:
    ...
    try:
        return safe_iloc_zero(
            vehicle_models, condition, context=f"vehicle with ID '{vehicle_id}'"
        )
...
def safe_calculate(
    func: callable, *args, context: str = "calculation", **kwargs
) -> Any:
    ...
    try:
        ...
        raise CalculationError(
            f"Failed to calculate {context}: {str(e)}", calculation_type=context
        ) from e


def validate_dataframe_columns(
    df: pd.DataFrame, required_columns: List[str], context: str = "DataFrame"
) -> None:
...
def safe_get_charging_option(
    charging_data: pd.DataFrame, charging_id: str
) -> pd.Series:
    ...
    return safe_iloc_zero(
        charging_data, condition, context=f"charging option with ID '{charging_id}'"
    )
