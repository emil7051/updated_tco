...
class ParameterInputBuilder:
    ...
    def collect_financial_parameters(self, financial_params: pd.DataFrame) -> 'ParameterInputBuilder':
        """Collect financial parameters from UI."""
        fin_params = FinancialParameters(financial_params)
        
        self.parameters['discount_rate'] = st.slider(
            'Discount Rate (%)',
            0.0, 15.0,
        ...
        self.parameters[ParameterKeys.CARBON_PRICE] = st.number_input(
            ...
            0, 500,
            int(fin_params.carbon_price),
            5
        )
        
        return self
    
    def collect_battery_parameters(self, battery_params: pd.DataFrame) -> 'ParameterInputBuilder':
        """Collect battery parameters from UI."""
        bat_params = BatteryParameters(battery_params)
        
        self.parameters['degradation_rate'] = st.slider(
