...
class CalculationOrchestrator:
    ...
    def _build_calculation_request(self, vehicle_id: str) -> CalculationRequest:
        ...
        parameters = CalculationParameters(
            ...
            selected_charging_profile_id=self.ui_context['selected_charging'], # Assuming this is the ID
            selected_infrastructure_id=self.ui_context['selected_infrastructure'], # Assuming this is the ID
            scenario_name=self.ui_context.get('scenario_meta', {}).get('name', 'Default'),
            diesel_price_override=self.ui_context.get(ParameterKeys.DIESEL_PRICE), # Using ParameterKeys for consistency
            ...
            degradation_rate_override=self.ui_context.get('battery_degradation_rate_override'), # Example key
            replacement_cost_override=self.ui_context.get('battery_replacement_cost_override')  # Example key
        )
    ...
    def _apply_ui_overrides_to_financial_params(self, financial_params_df: pd.DataFrame, calc_params: CalculationParameters) -> pd.DataFrame:
        ...
        if calc_params.diesel_price_override is not None:
            mask = df_copy[DataColumns.FINANCE_DESCRIPTION] == ParameterKeys.DIESEL_PRICE
            df_copy.loc[mask, DataColumns.FINANCE_DEFAULT_VALUE] = calc_params.diesel_price_override
        if calc_params.carbon_price_override is not None:
            mask = df_copy[DataColumns.FINANCE_DESCRIPTION] == ParameterKeys.CARBON_PRICE
            df_copy.loc[mask, DataColumns.FINANCE_DEFAULT_VALUE] = calc_params.carbon_price_override
        # Add more overrides here if needed
        return df_copy

    def _apply_ui_overrides_to_battery_params(self, battery_params_df: pd.DataFrame, calc_params: CalculationParameters) -> pd.DataFrame:
        """Apply UI overrides to a copy of the battery parameters DataFrame."""
        if battery_params_df.empty:
            logger.warning("Battery parameters DataFrame is empty. Cannot apply UI overrides.")
            return pd.DataFrame()
        ...
        if description_col not in df_copy.columns or value_col not in df_copy.columns:
            logger.error(f"Battery parameters DataFrame is missing required columns: '{description_col}' or '{value_col}'. Cannot apply overrides.")
        ...
        if calc_params.degradation_rate_override is not None:
            ...
            if mask.any():
                ...
                logger.debug(f"Applied degradation_rate_override: {calc_params.degradation_rate_override}")
            else:
                logger.warning(f"Parameter key '{ParameterKeys.DEGRADATION_RATE.value}' not found in battery_params_df description column.")
        ...
        if calc_params.replacement_cost_override is not None:
            ...
            if mask.any():
                ...
                logger.debug(f"Applied replacement_cost_override: {calc_params.replacement_cost_override}")
            else:
                logger.warning(f"Parameter key '{ParameterKeys.REPLACEMENT_COST.value}' not found in battery_params_df description column.")
    ...
    def perform_calculations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Calculation error during TCO calculation: {e}")
    ...
    def _transform_single_tco_result_for_ui(self, tco_result: TCOResult, request: CalculationRequest) -> Dict[str, Any]:
        """Transforms a single TCOResult into the dict structure expected by the UI."""
        if tco_result is None or request is None: return {}
        ...
        annual_maint = tco_result.annual_costs_breakdown.get('annual_maintenance_cost', 0.0)
        ...
        lifetime_emissions = tco_result.emissions_breakdown.get('lifetime_emissions', 0.0)
        ...
        tco_sub_dict = {
            ...
            'tco_annual': tco_result.tco_total_lifetime / request.parameters.truck_life_years if request.parameters.truck_life_years > 0 else 0
        }
        ...
        result_dict = {
            ...
            'scenario': {'name': request.parameters.scenario_name}, # Matches TCOResult.scenario_meta
        ...
        if request.drivetrain == Drivetrain.BEV and not request.infrastructure_options.empty:
            ...
            if not selected_infra_series.empty:
                 result_dict['selected_infrastructure_description'] = selected_infra_series.iloc[0].get(DataColumns.INFRASTRUCTURE_DESCRIPTION)

        return result_dict

    def _transform_results_for_ui(self, comparison: ComparisonResult, bev_request: CalculationRequest, diesel_request: CalculationRequest) -> Dict[str, Any]:
        """Transform service results to UI-expected format."""
        
        bev_ui_results = self._transform_single_tco_result_for_ui(comparison.base_vehicle_result, bev_request)
        diesel_ui_results = self._transform_single_tco_result_for_ui(comparison.comparison_vehicle_result, diesel_request)
