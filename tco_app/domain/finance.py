...
def calculate_annual_costs(
	...
	if apply_incentives and incentives_data is not None and vehicle_data[DataColumns.VEHICLE_DRIVETRAIN] == Drivetrain.BEV:
		...
		if not registration_exemption.empty:
			reg_exemption_condition = pd.Series([True] * len(registration_exemption), index=registration_exemption.index)
			reg_exemption_data = safe_iloc_zero(registration_exemption, reg_exemption_condition, context="registration exemption incentive")
		...
		if not insurance_discount.empty:
			insurance_condition = pd.Series([True] * len(insurance_discount), index=insurance_discount.index)
			insurance_data = safe_iloc_zero(insurance_discount, insurance_condition, context="insurance discount incentive")
		...
		if not electricity_discount.empty:
			electricity_condition = pd.Series([True] * len(electricity_discount), index=electricity_discount.index)
			electricity_data = safe_iloc_zero(electricity_discount, electricity_condition, context="electricity discount incentive")
...
def calculate_acquisition_cost(
	...
	if apply_incentives and vehicle_data[DataColumns.VEHICLE_DRIVETRAIN] == Drivetrain.BEV:
		...
		if not purchase_rebate.empty:
			rebate_condition = pd.Series([True] * len(purchase_rebate), index=purchase_rebate.index)
			rebate_data = safe_iloc_zero(purchase_rebate, rebate_condition, context="purchase rebate incentive")
		...
		if not stamp_duty_exemption.empty:
			duty_condition = pd.Series([True] * len(stamp_duty_exemption), index=stamp_duty_exemption.index)
			duty_exemption_data = safe_iloc_zero(stamp_duty_exemption, duty_condition, context="stamp duty exemption incentive")
...
def calculate_tco(
	...
	tco_per_tonne_km = safe_division(tco_per_km, vehicle_data[DataColumns.PAYLOAD_T], context="tco_per_km/payload calculation")
	...
	return {
		...
		'tco_annual': safe_division(npv_total_cost, truck_life_years, context="npv_total_cost/truck_life_years calculation"),
	}
...
def calculate_infrastructure_costs(
	...
	annual_capital = safe_division(price, service_life, context="price/service_life calculation")
	total_annual_cost = annual_capital + annual_maintenance
	per_vehicle_annual = safe_division(total_annual_cost, fleet_size, context="total_annual_cost/fleet_size calculation")

	replacement_cycles = max(1, math.ceil(safe_division(truck_life_years, service_life, context="truck_life_years/service_life calculation")))
	npv_infra = 0.0
	...
	npv_per_vehicle = safe_division(npv_infra, fleet_size, context="npv_infra/fleet_size calculation")
...
def apply_infrastructure_incentives(
	...
	if not active.empty:
		...
		incentive_data = safe_iloc_zero(active, incentive_condition, context="charging infrastructure subsidy incentive")
		rate = incentive_data['incentive_rate']
		costs['infrastructure_price_with_incentives'] = costs[DataColumns.INFRASTRUCTURE_PRICE] * (1 - rate)
		...
		costs['infrastructure_price_with_incentives'] = costs[DataColumns.INFRASTRUCTURE_PRICE]
