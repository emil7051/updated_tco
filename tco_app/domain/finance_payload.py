...
def calculate_payload_penalty_costs(
    ...
    trips_multiplier = (
        safe_division(
            diesel_payload,
            bev_payload,
            context="diesel_payload/bev_payload calculation",
        )
        if bev_payload
        else 1
    )
    ...
    def get_param(key, default):
        return (
            ...
            if key in financial_params[DataColumns.FINANCE_DESCRIPTION].values
            else default
        )
    ...
    additional_operational_cost_lifetime = (
        additional_operational_cost_annual * truck_life_years
    )
    ...
    baseline_trips = safe_division(
        annual_kms,
        avg_trip_distance,
        context="annual_kms/avg_trip_distance calculation",
    )
    ...
    effective_payload_ratio = safe_division(
        bev_payload, diesel_payload, context="bev_payload/diesel_payload calculation"
    )
    bev_tco_per_effective_tonnekm = (
        bev_results["tco"]["tco_per_tonne_km"] / effective_payload_ratio
    )
    bev_adjusted_lifetime_tco = (
        bev_results["tco"]["npv_total_cost"] + additional_operational_cost_lifetime
    )
    ...
    return {
        ...
        "additional_bevs_needed_per_diesel": (
            (
                safe_division(
                    diesel_payload,
                    bev_payload,
